...
if [ ${num_job_minus_1} -ge 0 ];then
	for rank in $(seq 0 $((num_job - 1))); do
		...
		CUDA_VISIBLE_DEVICES=${gpu_id} python3 pyscripts/run_wer.py $thread_dir/thread-0$rank.lst $sub_score_file $lang "asr" $whisper_path $paraformer_path &
	done
fi
wait
...
if [[ "$task_category" == "semantic" ]]; then
