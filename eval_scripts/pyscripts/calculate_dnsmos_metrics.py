# ported from https://github.com/urgent-challenge/urgent2025_challenge/blob/main/evaluation_metrics/calculate_nonintrusive_dnsmos.py
...
def dnsmos_metric(model, audio, fs=16000):
    ...
    return float(dnsmos_score["OVRL"]), float(dnsmos_score["SIG"]), float(dnsmos_score["BAK"])
...
def main(args):
    ...
    writers = {metric: (outdir / f"{metric}{suffix}.scp").open("w") for metric in METRICS}
    ...
    if not Path(args.primary_model).exists():
        raise ValueError(
            ...
            "and put it under the dnsmos folder."
        )
...
def process_one_pair(data_pair, model=None):
    ...
    for metric in METRICS:
        if metric == "DNSMOS_OVRL":
            scores[metric], scores["DNSMOS_SIG"], scores["DNSMOS_BAK"] = dnsmos_metric(
                model, inf, fs=fs
            )
        elif metric == "DNSMOS_SIG" or metric == "DNSMOS_BAK":
            continue
        else:
