...
class DNSMOS_local:
    ...
    def __init__(self, primary_model_path, p808_model_path, use_gpu=False, convert_to_torch=False):
    ...
    def audio_melspec(
        ...
        if self.convert_to_torch:
            ...
            if to_db:
                self.to_db.db_multiplier = math.log10(max(self.to_db.amin, torch.max(mel_spec)))
    ...
    def __call__(self, aud, input_fs, is_personalized_MOS=False):
        ...
        for idx in range(num_hops):
            ...
            if self.convert_to_torch:
                ...
                p808_input_features = self.audio_melspec(audio=audio_seg[:-160]).float()[None, :, :]
